...
model_name = f'RNN_ExerciseRecog_{batchSize}_{epochs}.h5'
logDir = os.path.join('Logs', 'RNN', model_name)
model_path = os.path.join('Models', 'RNN', model_name)
metrics_path = os.path.join('Metrics', 'RNN', model_name.replace('.h5', ''))
...
if os.path.exists(model_path):
    ...
    # Definizione dell'architettura della rete neurale RNN
    model = Sequential()
    model.add(SimpleRNN(128, input_shape=(windowsSize, 67), return_sequences=True))
...
plot_model(model, to_file=os.path.join(metrics_path, 'rnn_model.png'), show_shapes=True, show_layer_names=True)
