...
model_name = f'GRU_ExerciseRecog_{batchSize}_{epochs}.h5'
logDir = os.path.join('Logs', 'GRU', model_name)
model_path = os.path.join('Models', 'GRU', model_name)
metrics_path = os.path.join('Metrics', 'GRU', model_name.replace('.h5', ''))
...
if os.path.exists(model_path):
    ...
    model = Sequential()
    model.add(GRU(128, input_shape=(windowsSize, 67), return_sequences=True))
...
plot_model(model, to_file=os.path.join(metrics_path, 'gru_model.png'), show_shapes=True, show_layer_names=True)
